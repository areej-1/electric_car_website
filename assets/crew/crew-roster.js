 ...
 *
 * This is a classic script, not a module, so the walk feature also works when
 * the page is opened straight from disk (file:// blocks ES modules).
 */
window.CobrasCrew = (() => {
  const CREW = [
  ...
  const byName = (name) => CREW.find((m) => m.name === name);
